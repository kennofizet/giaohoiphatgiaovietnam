...
<div class="panel panel-default">
  <form class="w3-container w3-card-4" id="form_admin_official_edit" action="{{route('api.admin.official.edit')}}" method="POST">
  {{csrf_field()}}
  	<table class="table table-striped table-bordered table-hover">
  		<tbody>
  			<tr>
  				<th>Mã số thẻ</th>
  				<td colspan="2">
  					<input class="w3-input w3-border-0" type="text" name="card_number" value="{{$detail_official_edit->card_number}}">
  				</td>
  			</tr>
  			<tr>
  				<th>Họ tên *</th>
  				<td>
  					<input class="w3-input w3-border-0" type="text" name="name" value="{{$detail_official_edit->name}}">
  				</td>
  				<td colspan="2" rowspan="3" align="center">
            <div class="text-left">
              <div class="preview prev-restyle img-wrapper"></div>
              <hr>
              <div class="file-upload-wrapper">
                  <input type="file" name="photo_profile" class="file-upload-native" accept="image/*" />
                  <input type="text" disabled placeholder="Tải tại đây @if($detail_official_edit->photo_profile) (Cập nhật) @else (Thêm mới) @endif" class="file-upload-text" />
              </div>
            </div>
  				</td>
  			</tr>
  			<tr>
  				<th>Pháp Danh</th>
  				<td>
  					<input class="w3-input w3-border-0" type="text" name="alias" value="{{$detail_official_edit->alias}}">
  				</td>			
  			</tr>
  			<tr>
  				<th>Thể loại <button type="button" class="btn btn-primary" style="margin-left: 10px" data-toggle="modal" data-target="#adminOfficialRedirectCategoryModal">New + </button></th>
	          <!-- Modal -->
	          <div class="modal fade" id="adminOfficialRedirectCategoryModal" tabindex="-1" role="dialog" aria-labelledby="adminOfficialRedirectCategoryModalLabel" aria-hidden="true">
	            <div class="modal-dialog" role="document">
	              <div class="modal-content">
	                <div class="modal-header">
	                  <h5 class="modal-title" id="adminOfficialRedirectCategoryModalLabel">Xác nhận chuyển hướng</h5>
	                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	                    <span aria-hidden="true">&times;</span>
	                  </button>
	                </div>
	                <div class="modal-body">
	                  Những thông tin bạn nhập sẽ không được lưu lại, bạn sẽ được chuyển hướng qua trang thêm mới Thể loại
	                </div>
	                <div class="modal-footer">
	                  <button type="button" class="btn btn-secondary" data-dismiss="modal">Hủy</button>
	                  <button type="button" class="btn btn-primary"><a href="{{route('admin.official.category.create')}}" style="color: white">Chuyển hướng</a></button>
	                </div>
	              </div>
	            </div>
	          </div>
  				<td>
  					<select class="form-control form-control-xs selectpicker" name="category" data-size="7" data-live-search="true" data-title="Thể loại" id="category" data-width="100%">
              <option value="" selected>Chọn Thể loại</option>
              @foreach($list_official_category as $detail_category)
              <option value="{{$detail_category->id}}" @if($detail_category->id == $detail_official_edit->category ) selected @endif>{{$detail_category->name}}</option>
              @endforeach
            </select>
  				</td>
  			</tr>
  			<tr>
  				<th>Ngày sinh</th>
  				<td colspan="2">
  					<input class="w3-input w3-border-0" type="text" name="date_of_birth" value="{{$detail_official_edit->date_of_birth}}">
  				</td>
  			</tr>
  			<tr>
  				<th>Số CMND</th>
  				<td colspan="2">
  					<input class="w3-input w3-border-0" type="text" name="cmnd" value="{{$detail_official_edit->cmnd}}">
  				</td>
  			</tr>
  			<tr>
  				<th>Chức vụ <button type="button" class="btn btn-primary" style="margin-left: 10px" data-toggle="modal" data-target="#adminOfficialRedirectPositionModal">New + </button></th>
          <!-- Modal -->
          <div class="modal fade" id="adminOfficialRedirectPositionModal" tabindex="-1" role="dialog" aria-labelledby="adminOfficialRedirectPositionModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="adminOfficialRedirectPositionModalLabel">Xác nhận chuyển hướng</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
                <div class="modal-body">
                  Những thông tin bạn nhập sẽ không được lưu lại, bạn sẽ được chuyển hướng qua trang thêm mới Chức vụ
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-dismiss="modal">Hủy</button>
                  <button type="button" class="btn btn-primary"><a href="{{route('admin.official.position.create')}}" style="color: white">Chuyển hướng</a></button>
                </div>
              </div>
            </div>
          </div>
  				<td colspan="2">
  					<!-- <input class="w3-input w3-border-0" type="text" name="position"> -->
  					<select class="form-control form-control-xs selectpicker" name="position" data-size="7" data-live-search="true" data-title="Chức vụ" id="position" data-width="100%">
              <option value="" selected>Chọn Chức vụ</option>
              @foreach($list_official_position as $detail_position)
              <option value="{{$detail_position->id}}" @if($detail_position->id == $detail_official_edit->position ) selected @endif>{{$detail_position->name}}</option>
              @endforeach
            </select>
  				</td>
  			</tr>
  			<tr>
  				<th>Đc thường trú</th>
  				<td colspan="2">
  					<input class="w3-input w3-border-0" type="text" name="address" value="{{$detail_official_edit->address}}">
  				</td>
  			</tr>
  			<tr>
              ...
              <div class="file-upload-wrapper">
                  <input type="file" name="qr_code" class="file-upload-native" accept="image/*" />
                  <input type="text" disabled placeholder="Tải tại đây @if($detail_official_edit->qr_code) (Cập nhật) @else (Thêm mới) @endif" class="file-upload-text" />
              </div>
  					</div>
  	...
  	<input type="number" name="id" value="{{$detail_official_edit->id}}" style="display: none;">
    <button class="btn btn-success" style="float: right;" type="submit">Lưu</button>
  </form>
</div>
@endsection
...
<script type="text/javascript">
  ...
  function maskImgs() {
    ...
    $.each($('.img-wrapper img'), function(index, img) {
      ...
      var parent_prev = $(($(img).parent()).parent().find('.file-upload-wrapper')[0]).css('display','none');
      // parent
  ...
  $('#tag_upload_avatar_oficial').on('click',function () {
    $('#hidden_upload_avatar_oficial').click();
  });
  $('#hidden_upload_avatar_oficial').on('change',function () {
    if($(this).val()){

    }
  });
  $('.preview').on('click',function () {
    $(($(this).next()).children().find('.file-upload-text')[0]).click();
  });
</script>
<script src="https://cdn.jsdelivr.net/npm/sweetalert2@10"></script>
<script type="text/javascript">
  $('#form_admin_official_edit').on('submit',function (e) {
    e.preventDefault();
    $('.mess-name').css('display','none');
      ...
      $.ajax({
        url:"{{route('api.admin.official.edit')}}",
        method:"POST",
        ...
        success:function(data){
          if (data.message === "success") {
            ...
            $('.mess-name').css('display','block');
              ...
              icon: 'error',
              title: 'Lỗi...',
              text: 'Vui lòng nhập đầy đủ thông tin có dấu *!',
